...
//#include "../../include/renderable/LakeRenderable.hpp"
...
TexturedLakeRenderable::TexturedLakeRenderable(
        ShaderProgramPtr shaderProgram, const std::string& textureFilename)
    : HierarchicalRenderable(shaderProgram),
      m_pBuffer(0), m_nBuffer(0), m_tBuffer(0), m_texId(0)
{
    std::cout << "Le lac des cygnes \n";
    // Initialize geometry
    teachers::getUnitLake(m_positions, m_normals, m_texCoords);
    
    //teachers::getUnitLake(m_positions, m_normals, m_origTexCoords);
    ...
    for (int i = 0; i < 25; i ++) {
        m_texCoords.push_back(glm::vec2(1.0, 1.0)); //close to the texture's lower left corner
        m_texCoords.push_back(glm::vec2(1.0, 0.0)); //lower right corner of the texture
        m_texCoords.push_back(glm::vec2(0.0, 1.0)); //middle of the top line of the texture
    }
...
void TexturedLakeRenderable::do_draw()
{
    //Locations
...
void TexturedLakeRenderable::do_animate(float time)
{
}

void TexturedLakeRenderable::setMaterial(const MaterialPtr& material)
{
    m_material = material;
}
