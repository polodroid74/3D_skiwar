...
//#include "../../include/renderable/IglooRenderable.hpp"
...
TexturedIglooRenderable::TexturedIglooRenderable(
        ShaderProgramPtr shaderProgram, const std::string& textureFilename)
    : HierarchicalRenderable(shaderProgram),
      m_pBuffer(0), m_nBuffer(0), m_tBuffer(0), m_texId(0)
{
    std::cout << "Un igloo sur la banquise ! \n";
    // Initialize geometry
    unsigned int strips = 10;
    unsigned int slices = 10;
    teachers::getUnitDemiSphere(m_positions, m_normals, strips, slices);
    
    //teachers::getUnitIgloo(m_positions, m_normals, m_origTexCoords);
    ...
    // On texture les facettes de la sphère 
    for (int i = 0; i < strips; i ++) {
        for (int j = 0; j < slices; j++) {
            m_texCoords.push_back(glm::vec2(0.0, 0.250)); 
            m_texCoords.push_back(glm::vec2(0.250, 0.250)); 
            m_texCoords.push_back(glm::vec2(0.250, 0.0)); 
    ...
    // On texture l'entrée de l'igloo
    for (unsigned int i = 0; i < slices; ++i) {
            // cotés du cylindre d'entrée
            m_texCoords.push_back(glm::vec2(0.250, 0.0)); 
            m_texCoords.push_back(glm::vec2(0.0, 0.125)); 
            m_texCoords.push_back(glm::vec2(0.250, 0.125)); 
...
void TexturedIglooRenderable::do_draw()
{
    //Locations
...
void TexturedIglooRenderable::do_animate(float time)
{
}

void TexturedIglooRenderable::setMaterial(const MaterialPtr& material)
{
    m_material = material;
}
