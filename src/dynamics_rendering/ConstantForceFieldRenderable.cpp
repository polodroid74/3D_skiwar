...
ConstantForceFieldRenderable::ConstantForceFieldRenderable(
        ShaderProgramPtr shaderProgram, ConstantForceFieldPtr forceField) :
    HierarchicalRenderable(shaderProgram),
    m_forceField(forceField), m_pBuffer(0), m_cBuffer(0)
{
    //Create geometric data
    const std::vector<TexturedMeshRenderablePhysicPtr>& meshes = m_forceField->getmeshes();
    m_positions.resize(2.0*meshes.size());
    m_colors.resize(2.0*meshes.size());
    m_normals.resize(2.0*meshes.size());
