...
void teachers::getUnitCylinder(std::vector<glm::vec3>& positions, std::vector<glm::vec3>& normals, std::vector<glm::vec2> &texCoords, const unsigned int &slices)
{
    size_t number_of_triangles = (slices+2) * 2;
    size_t number_of_vertices = number_of_triangles * 3;
    float angle_step = 2.0 * M_PI / double( slices );
...
*/
        ...
        positions[ voffset +  1 ] = glm::vec3{previous_cos, previous_sin, 1};
        positions[ voffset +  2 ] = glm::vec3{previous_cos, previous_sin, 0};
...
*/
        ...
        normals[ voffset +  1 ] = glm::vec3{previous_cos,previous_sin,0};
        normals[ voffset +  2 ] = glm::vec3{previous_cos,previous_sin,0};
...
/*
		...
		texCoords[ voffset + 0 ] = glm::vec2{1/4.0,0};
		texCoords[ voffset + 1 ] = glm::vec2{i/s,1/3.0};
		texCoords[ voffset + 2 ] = glm::vec2{(i+1)/s,1/3.0};
*/
		texCoords[ voffset + 0 ] = glm::vec2{(i+1)/s,2/3.0};
		texCoords[ voffset + 1 ] = glm::vec2{i/s,2/3.0};
		texCoords[ voffset + 2 ] = glm::vec2{i/s,1/3.0};

		texCoords[ voffset + 3 ] = glm::vec2{(i+1)/s,2/3.0};
		texCoords[ voffset + 4 ] = glm::vec2{i/s,1/3.0};
		texCoords[ voffset + 5 ] = glm::vec2{(i+1)/s,1/3.0};
/*		
		texCoords[ voffset + 9 ] = glm::vec2{i/s,i/s};
		texCoords[ voffset + 10 ] = glm::vec2{i/s,2/3.0};
		texCoords[ voffset + 11 ] = glm::vec2{1/2.0,0};
  */  }
/*
		slices+=2;
	positions[ slices*2 +  0 ] = glm::vec3{0,0,1};
	positions[ slices*2 +  1 ] = glm::vec3{1,0,1};
	positions[ slices*2 +  2 ] = glm::vec3{1,1,1};

	positions[ slices*2 +  3 ] = glm::vec3{1,1,1};
	positions[ slices*2 +  4 ] = glm::vec3{0,1,1};
	positions[ slices*2 +  5 ] = glm::vec3{0,0,1};

	normals[ slices*2 +  0 ] = glm::vec3{0,0,1};
	normals[ slices*2 +  1 ] = glm::vec3{0,0,1};
	normals[ slices*2 +  2 ] = glm::vec3{0,0,1};

	normals[ slices*2 +  3 ] = glm::vec3{0,0,1};
	normals[ slices*2 +  4 ] = glm::vec3{0,0,1};
	normals[ slices*2 +  5 ] = glm::vec3{0,0,1};

	texCoords[ slices*2 + 0 ] = glm::vec2{1/4.0,1/3.0};
	texCoords[ slices*2 + 1 ] = glm::vec2{1/4.0,0};
	texCoords[ slices*2 + 2 ] = glm::vec2{1/2.0,0};

	texCoords[ slices*2 + 3 ] = glm::vec2{1/2.0,0};
	texCoords[ slices*2 + 4 ] = glm::vec2{1/2.0,1/3.0};
	texCoords[ slices*2 + 5 ] = glm::vec2{1/4.0,1/3.0};
*/
...
void teachers::getUnitLake(std::vector<glm::vec3>& positions, std::vector<glm::vec3>& normals, std::vector<glm::vec2>& texCoords)
{
    unsigned int nbSlices = 25;
    glm::vec3 center(0.0, 0.0, 0.0);
    ...
    for (unsigned int i = 0; i < nbSlices; ++i) {
        ...
        positions.push_back(glm::vec3(currentCos, currentSin, 0.0));
        positions.push_back(glm::vec3(nextCos, nextSin, 0.0));
        /*
    texCoords.push_back(glm::vec2(0.0,0.0));
    texCoords.push_back(glm::vec2(1.0,1.0));
    texCoords.push_back(glm::vec2(0.0,1.0));
...
void teachers::getUnitDemiSphere(std::vector<glm::vec3>& positions, std::vector<glm::vec3>& normals, const unsigned int& strips, const unsigned int& slices)
{
    double radius=1.0;
    glm::vec3 center(0.0,0.0,0.0);
    ...
    for(int i=0; i<slices; ++i)
    {
        ...
        {
            double curr_theta = i*(2.0*M_PI/(double)slices);
            double curr_phi = j*((M_PI/2.0)/(double)strips);

            double next_theta = (i+1)*(2.0*M_PI/(double)slices);
            double next_phi = (j+1)*((M_PI/2.0)/(double)strips);
    ...
    double deb = 0.8;
    double fin = 1.2;
    glm::vec3 bottomCenter(deb, 0.0, 0.0);
    glm::vec3 topCenter(fin, 0.0, 0.0);

    
    double r = 0.25;
    
    double dTheta = 1.0 * M_PI / (double) slices;
    double currentTheta = 0.0;
    double currentCos = r*1.0;
    double currentSin = 0.0;

    for (unsigned int i = 0; i < slices; ++i) {
        double nextCos = r*cos(currentTheta + dTheta);
        double nextSin = r*sin(currentTheta + dTheta);
        ...
        positions.push_back(glm::vec3(deb, currentCos, currentSin));
        positions.push_back(glm::vec3(fin, nextCos, nextSin));
        positions.push_back(glm::vec3(deb, nextCos, nextSin));
        for (unsigned int i = 0; i < 3; i++) {
        ...
        positions.push_back(glm::vec3(deb, currentCos, currentSin));
        positions.push_back(glm::vec3(fin, nextCos, nextSin));
        positions.push_back(glm::vec3(fin, currentCos, currentSin));
        for (unsigned int i = 0; i < 3; i++) {
            normals.push_back(normal);
        }
        /*
